#Dr. Andrew Steinberg
#COP3223 Student Python Script v2.0
...
qinputs5 = ['516.18\n1.23\n-7\n5\ny\n-1\nN\n']
qinputs6 = ['10.97\n4.00\n10\n6\nY\n12\nA\n']
testcases = [qinputs1, qinputs2, qinputs3, qinputs4, qinputs5, qinputs6]
...
def result():
    ...
    for test in testcases:
        f1 = open(cwd + '/samplesolutionsp3testcase' + str(scenario) + '.txt', "r")
        f2 = open('sp3student_output_test_case_' + str(scenario) + '.txt', "r")
...
def compilerunfile():
    ...
    #gcc compile command the executable will be smallprogram 1 instead of a.out

    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram3' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram3 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram4
    ...
    for test in testcases:
        ...
        #list comprehension
        inputs = ' '.join([str(item) for item in test])
        f = open('sp3student_output_test_case_' + str(scenario) + '.txt', "w")
        f.write("----------------------------------------------------------------------\n")
...
def setupchecker():
    ...
    for x in range(1, 7):
        if not os.path.exists(cwd + "/samplesolutionsp3testcase" + str(x) + ".txt"):
